...
main()
{
  ...
  # Status left
  if $show_powerline; then
    tmux set-option -g status-left "#[fg=${green}]#{?client_prefix,#[fg=${yellow}],}#[bg=${green},fg=${black},bold]#{?client_prefix,#[bg=${yellow}],}${left_sep} ${left_icon} #[fg=${green},]#{?client_prefix,#[fg=${yellow}],}${left_sep}"
    powerbg=default
  else
