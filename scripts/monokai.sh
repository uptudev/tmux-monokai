...
main()
{
  ...
  # Status left
  if $show_powerline; then
    tmux set-option -g status-left "#[bg=${green}]#{?client_prefix,#[fg=${yellow}],}#[bg=${green},fg=${black},bold]#{?client_prefix,#[bg=${yellow}],} ${left_icon} #[fg=${green},bg=#{default}]#{?client_prefix,#[fg=${yellow}],}${left_sep}"
    powerbg=default
  else
  ...
  if $show_powerline; then
    tmux set-option -ga status-right "#[fg=${green},bg=${magenta}]${right_sep}#[bg=${green},fg=${black},bold] #h #[fg=${green},]"
  else
